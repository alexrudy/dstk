...
def test_unconstrained_logit():
    ...
    np.testing.assert_allclose(accuracy_score(target, np.round(clr.predict_proba(scaled_data).squeeze())), 0.978, atol=1e-2)
